...
async function renderHome(){
 ...
 if(profile.role==='participant'){
   ...
   const byPid={};
   active.forEach(p=>byPid[p.id]={p,done:0,load:0,w:null,t0:0});
...
async function renderWellness(){
  ...
  view.innerHTML=`<div class="card"><div class="sectionTitle"><div><h2>Wellness / readiness</h2><p class="muted">Una sola compilazione al giorno per partecipante.</p></div><span class="sectionPill">Readiness</span></div>${alreadyToday?'<p class="note"><b>Wellness di oggi già registrato.</b> Potrai compilarne uno nuovo domani.</p>':`<form id="wellForm">${pf}${wellnessScale('Qualità del sonno','sleep',['Pessima','Scarsa','Discreta','Buona','Ottima'])}${wellnessScale('Stanchezza generale','fatigue',['Molto alta','Alta','Media','Bassa','Nessuna'])}${wellnessScale('DOMS','doms',['Molto elevati','Elevati','Moderati','Lievi','Nessuno'])}${wellnessScale('Stress percepito','stress',['Molto alto','Alto','Medio','Basso','Molto basso'])}<label><input type="checkbox" id="painPresent"> Dolore/problema fisico oggi</label><div class="grid two"><label>Sede<input id="painSite"></label><label>Intensità 0-10<input id="painScore" type="number" min="0" max="10"></label></div><div class="toolbar"><button class="primary" id="wellSaveBtn">Salva wellness</button><button type="button" class="secondary hidden" id="wellCancelEdit">Annulla modifica</button></div></form>`}</div><div id="wellList"></div>`;
  let editingWellId=null;
  const submitWell=async e=>{e.preventDefault();const participant_id=$('#participantId').value;const payload={participant_id,wellness_date:today,sleep:+$('#sleep').value,fatigue:+$('#fatigue').value,doms:+$('#doms').value,stress:+$('#stress').value,pain_present:$('#painPresent').checked,pain_site:$('#painSite').value.trim()||null,pain_score:$('#painPresent').checked?(+$('#painScore').value||0):null};let error;if(editingWellId){({error}=await sb.from('wellness').update(payload).eq('id',editingWellId));}else{({error}=await sb.from('wellness').insert(payload));}if(error){if(error.code==='23505'||/duplicate|unique/i.test(error.message||''))return toast('Wellness di oggi già registrato');return toast(error.message)}toast(editingWellId?'Wellness aggiornato':'Wellness salvato');await renderWellness()};
  if($('#wellForm')) $('#wellForm').onsubmit=submitWell;
  let q=sb.from('wellness').select('*,participants(code)').order('recorded_at',{ascending:false}).limit(40);if(profile.role==='participant')q=q.eq('participant_id',participantProfile.id);const {data}=await q;
  $('#wellList').innerHTML=`<div class="card"><h2>Ultimi wellness</h2><div class="tableWrap"><table><thead><tr><th>Data</th><th>Partecipante</th><th>Score</th><th>Dolore</th>${profile.role==='owner'?'<th>Azioni</th>':''}</tr></thead><tbody>${(data||[]).map(x=>{const p=partMap[x.participant_id]||{id:x.participant_id,code:x.participants?.code||''};return `<tr><td>${fmtDate(x.wellness_date)||fmt(x.recorded_at)}</td><td><b>${esc(participantLabel(p,ids))}</b></td><td><span class="scoreChip">${x.score}/20</span></td><td>${x.pain_present?`${esc(x.pain_site||'')} ${x.pain_score??'—'}/10`:'No'}</td>${profile.role==='owner'?`<td><div class="rowActions"><button class="secondary miniDelete" data-edit-well="${x.id}">Modifica</button><button class="danger miniDelete" data-del-well="${x.id}">Elimina</button></div></td>`:''}</tr>`}).join('')}</tbody></table></div></div>`;
  document.querySelectorAll('[data-edit-well]').forEach(b=>b.onclick=async()=>{const row=(data||[]).find(x=>x.id===b.dataset.editWell);if(!row||!$('#wellForm'))return;editingWellId=row.id;$('#participantId').value=row.participant_id;$('#sleep').value=row.sleep;$('#fatigue').value=row.fatigue;$('#doms').value=row.doms;$('#stress').value=row.stress;$('#painPresent').checked=!!row.pain_present;$('#painSite').value=row.pain_site||'';$('#painScore').value=row.pain_score??'';$('#wellSaveBtn').textContent='Aggiorna wellness';$('#wellCancelEdit').classList.remove('hidden');window.scrollTo({top:0,behavior:'smooth'})});
