...
async function renderWellness(){
  ...
  const targetPid=profile.role==='participant'?participantProfile.id:null;
  let todayQ=sb.from('wellness').select('id,participant_id,wellness_date').eq('wellness_date',today);
  if(targetPid) todayQ=todayQ.eq('participant_id',targetPid);
  const {data:todayRows}=await todayQ;
  const alreadyToday=profile.role==='participant' && (todayRows||[]).length>0;
  view.innerHTML=`<div class="card"><h2>Wellness / readiness</h2><p class="muted">Una sola compilazione al giorno per partecipante.</p>${alreadyToday?'<p class="note"><b>Wellness di oggi già registrato.</b> Potrai compilarne uno nuovo domani.</p>':`<form id="wellForm">${pf}${wellnessScale('Qualità del sonno','sleep',['Pessima','Scarsa','Discreta','Buona','Ottima'])}${wellnessScale('Stanchezza generale','fatigue',['Molto alta','Alta','Media','Bassa','Nessuna'])}${wellnessScale('DOMS','doms',['Molto elevati','Elevati','Moderati','Lievi','Nessuno'])}${wellnessScale('Stress percepito','stress',['Molto alto','Alto','Medio','Basso','Molto basso'])}<label><input type="checkbox" id="painPresent"> Dolore/problema fisico oggi</label><div class="grid two"><label>Sede<input id="painSite"></label><label>Intensità 0-10<input id="painScore" type="number" min="0" max="10"></label></div><button class="primary">Salva wellness</button></form>`}</div><div id="wellList"></div>`;
  if($('#wellForm')) $('#wellForm').onsubmit=async e=>{
    e.preventDefault();
    const participant_id=$('#participantId').value;
    const payload={participant_id,wellness_date:today,sleep:+$('#sleep').value,fatigue:+$('#fatigue').value,doms:+$('#doms').value,stress:+$('#stress').value,pain_present:$('#painPresent').checked,pain_site:$('#painSite').value.trim()||null,pain_score:$('#painPresent').checked?(+$('#painScore').value||0):null};
    const {error}=await sb.from('wellness').insert(payload);
    if(error){ if(error.code==='23505'||/duplicate|unique/i.test(error.message||'')) return toast('Wellness di oggi già registrato'); return toast(error.message); }
    toast('Wellness salvato'); await renderWellness();
  };
  let q=sb.from('wellness').select('*,participants(code)').order('recorded_at',{ascending:false}).limit(30);
  if(profile.role==='participant') q=q.eq('participant_id',participantProfile.id);
  const {data}=await q;
  $('#wellList').innerHTML=`<div class="card"><h2>Ultimi wellness</h2><div class="tableWrap"><table><thead><tr><th>Data</th><th>Codice</th><th>Score</th><th>Dolore</th>${profile.role==='owner'?'<th>Azioni</th>':''}</tr></thead><tbody>${(data||[]).map(x=>`<tr><td>${fmtDate(x.wellness_date)||fmt(x.recorded_at)}</td><td>${x.participants?.code||''}</td><td>${x.score}/20</td><td>${x.pain_present?`${esc(x.pain_site||'')} ${x.pain_score??'—'}/10`:'No'}</td>${profile.role==='owner'?`<td><button class="danger miniDelete" data-del-well="${x.id}">Elimina</button></td>`:''}</tr>`).join('')}</tbody></table></div></div>`;
...
async function renderTraining(){
  ...
  $('#sessionId').onchange=updatePlan; updatePlan();
  $('#logForm').onsubmit=async e=>{e.preventDefault();const payload={participant_id:$('#participantId').value,session_template_id:$('#sessionId').value,completed:$('#completed').checked,duration_min:+$('#duration').value,srpe:+$('#srpe').value,actual_work:$('#actualWork').value.trim()||null,pain_post:$('#painPost').checked,pain_site:$('#painPostSite').value.trim()||null,pain_score:$('#painPost').checked?(+$('#painPostScore').value||0):null,notes:$('#logNotes').value.trim()||null,performed_at:new Date().toISOString()};const {error}=await sb.from('session_logs').upsert(payload,{onConflict:'participant_id,session_template_id'});if(error)return toast(error.message);toast('Seduta salvata');await renderTraining()};
  let q=sb.from('session_logs').select('*,participants(code),session_templates(session_number,session_type,week_number)').order('performed_at',{ascending:false}).limit(40);if(profile.role==='participant')q=q.eq('participant_id',participantProfile.id);const {data:logs}=await q;
  $('#logList').innerHTML=`<div class="card"><h2>Ultime sedute</h2><div class="tableWrap"><table><thead><tr><th>Codice</th><th>Seduta</th><th>Tipo</th><th>Durata</th><th>sRPE</th><th>TL</th>${profile.role==='owner'?'<th>Azioni</th>':''}</tr></thead><tbody>${(logs||[]).map(x=>`<tr><td>${x.participants?.code||''}</td><td>${x.session_templates?.session_number||''}</td><td>${x.session_templates?.session_type||''}</td><td>${x.duration_min??'—'}'</td><td>${x.srpe??'—'}</td><td>${x.session_load?Math.round(x.session_load)+' AU':'—'}</td>${profile.role==='owner'?`<td><button class="danger miniDelete" data-del-log="${x.id}">Elimina</button></td>`:''}</tr>`).join('')}</tbody></table></div></div>`;
