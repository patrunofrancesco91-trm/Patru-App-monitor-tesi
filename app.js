...
async function renderAthleteArea(){
 ...
 const load=async()=>{
   ...
   const [{data:logs,error:le},{data:wells,error:we},{data:tests,error:te}]=await Promise.all([
   ...
   if(le||we||te){$('#athleteAreaBody').innerHTML=`<div class="card"><p class="msg">${esc((le||we||te).message)}</p></div>`;return}
   const ALLL=logs||[],ALLW=wells||[],T=tests||[];
   ...
   if(section==='training') content=`<div class="card"><div class="sectionTitle"><div><h2>Andamento Training Load</h2><p class="muted">Impatto interno delle sedute nel periodo selezionato.</p></div><span class="sectionPill">${Math.round(totalAU)} AU</span></div>${svgLineChart(loadRows,[{key:'au',label:'AU'}],{title:'Carico AU',min:0})}</div><div class="card"><div class="trainingCardGrid">${trainCards||'<p class="muted">Nessun allenamento nel periodo.</p>'}</div></div>`;
   if(section==='wellness') content=`<div class="analyticsGrid"><div class="card"><h2>Wellness totale</h2>${svgLineChart(wellRows,[{key:'score',label:'Score'}],{title:'Wellness score',min:4,max:20})}</div><div class="card"><h2>Componenti wellness</h2>${svgLineChart(wellRows,[{key:'sleep',label:'Sonno'},{key:'fatigue',label:'Stanchezza'},{key:'doms',label:'DOMS'},{key:'stress',label:'Stress'}],{title:'Componenti wellness',min:1,max:5})}</div></div><div class="card"><div class="wellnessCardGrid compact">${wellCards||'<p class="muted">Nessun wellness nel periodo.</p>'}</div></div>`;
