...
async function renderAthleteArea(){
 ...
 const load=async()=>{
   ...
   <div class="card"><div class="sectionTitle"><div><h2>Dettaglio wellness</h2><p class="muted">Ultime rilevazioni con punteggi singoli.</p></div></div><div class="wellnessCardGrid compact">${[...W].reverse().slice(0,12).map(x=>`<article class="wellnessResultCard"><div class="wellnessResultHead"><div><b>${fmtDate(x.wellness_date)}</b></div><div class="wellnessTotal"><strong>${x.score}</strong><span>/20</span></div></div><div class="wellnessMetricGrid"><div><span>Sonno</span><b>${x.sleep}/5</b></div><div><span>Stanchezza</span><b>${x.fatigue}/5</b></div><div><span>DOMS</span><b>${x.doms}/5</b></div><div><span>Stress</span><b>${x.stress}/5</b></div></div></article>`).join('')||'<p class="muted">Nessun wellness registrato.</p>'}</div></div>
